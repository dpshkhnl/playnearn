...
if(isset($user_signup) && isset($access_key)) {
	...
	$register_user = "INSERT INTO users (name,username,password,email,fcm_id,points,refer,ip_address,phone,image) VALUES ('$name','$username','$pwd','$email','$fcm_id','$points', '$refer','$ip_address','$phone','$image')";
	...
	if($send_server){
		...
		$response['id'] = mysqli_insert_id($link);
			...
			$to = $email;
			$subject = "Login Details | Khela App ";
			$text = "Hello Dear $username, your Login Details for Khela App";
			$text .= "<br>Username : <b>".$username."</b>";
			$text .= "<br>Password : <b>".$password."</b>";
			$text .= "<br><br>Please login via following Username and password now. <br>This Mail was Signup Khela App by you <br><br><b>Khela App</b>";
			$header = "From: Login Details | Khela App<$from>\r\nReply-to: $from\r\n";
...
if(isset($access_key) && isset($leaderboard)){
	...
	$sql = mysqli_query($link,"SELECT points,username,image FROM users ORDER BY points DESC limit 10");
	...
	if($key != $access_key){
		...
		$pwd = md5($current_password);
		    if($result['password']==$pwd) {
			    $sql = mysqli_query($link,"UPDATE `users` SET `password`='".md5($new_pass)."'`email`='".$email."' WHERE `email`='".$email."'");
		  		$response['error'] = "false";
...
if(isset($access_key) && isset($update_badge)){
	...
	$sql= mysqli_query($link, "UPDATE users SET ip_address = 0 WHERE username='$username'");
